...
| `d5f9751` | **Not mine - the frapp-gh lane**, doc-only. **Verified by me, and both were real overclaims.** (1) The week table said "ranking board created" flatly; `openSession` in `frapp-gh/src/handlers/cron-handlers.ts:106-121` wraps board creation in try/catch, logs a warning and carries on with `projectId` undefined, so a period proceeds normally without a board. (2) The intro called the payout "the ZAO 2x Fibonacci curve" as a property of the game; the ledger shows periods 67-70 paid standard Fibonacci and two later periods paid neither curve, which is the same finding behind `5f0ef2c`. Both fixes are accurate, the diff touches only `frapp-gh/README.md`, and no code changed. |
