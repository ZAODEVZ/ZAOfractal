...
"Either curve" is not a hedge. ZAO has paid two payout curves - standard Fibonacci for its first four settled periods, the 2x curve since (Chapter 8 gives the history from the ledger) - and their Gini is **identical**, because the two are exact scalar multiples of each other and Gini does not change under scaling. That is a theorem rather than a coincidence, and it has a limit worth naming: it would stop holding for a curve that is not a multiple of the others, which the ledger has already produced twice in the flat payouts of periods 78 and 105. The escalation moved how fast the ledger accumulates, not how unequal a single week is. Which is why the 0.73 below, and not the 0.41 above, is the number this section is really about.
