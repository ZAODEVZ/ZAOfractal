...
The Respect Game is conceptually simple. Running it consistently each week requires reliable infrastructure.

In September 2026, the Discord bot layer underwent a fundamental architectural reset. The initial Python bot (`fractalbotapril2026`), which attempted to manage 52 slash commands across hats, proposals, and events within a single monolithic codebase, was retired following rate-limit instability and hosting failures.

The active production architecture is **zao-fractal-bot**, a modular TypeScript bot (discord.js, viem, Supabase) designed around a clean two-tier subsystem model:

**Subsystem 1 (Respect Game Core - Live in Production):**
- **Automated Roster Capture:** The bot observes voice channels, message activity, and reactions, resolving Discord members to their Optimism wallets and Farcaster identities.
- **Seating and Circle Assignment:** The bot partitions members into breakout circles of 4-6 participants, ensuring varied peer interactions while honoring group size constraints.
- **Fibonacci Scoring and Camera Tracking:** Awards are computed using ZAO's 2x Fibonacci scale (`[110, 68, 42, 26, 16, 10]`, or `40` for an even split). In addition, the bot continuously monitors voice state to automatically award the `+10` point bonus for camera-on participation without requiring manual facilitator tallies.
- **Embedded Knowledge System:** A built-in knowledge engine answers member queries about Respect rules, contract addresses, scoring, and governance parameters directly in Discord.
- **Pure Engine Testing:** All scoring, seating, and threshold algorithms are isolated as pure functions backed by an automated test suite.

**Subsystem 2 (Hats Protocol and Governance - Deferred Roadmap):**
Per founder ruling, complex role synchronization (Hats Protocol Tree 226 on Optimism), working group team management, and multi-signer on-chain OREC proposal execution are formally decoupled as Subsystem 2. They will be reintroduced as standalone micro-modules after the core weekly ceremony game engine settles in production.

By separating the ritual game engine (Subsystem 1) from the on-chain governance and role machinery (Subsystem 2), ZAO ensures that the weekly Monday ceremony runs reliably without technical failure.
